...
\item Let $A_1, A_2,...A_n\in\textbf{\textit{A}}$, with $A_i\cap A_j=\varnothing$ for $i\neq j$. $\sum_{k=1}^nP(A_k)=\sum_{k=1}^n\int 1_{A_k}\cdot fdP$. Using monotone convergence theorem, we know we can build $f_n\uparrow f$ with $f_n$ simple functions and so that $\int f_n dP\uparrow \int fdP$. It's $\sum_{k}\int 1_{A_k}\cdot fdP=\lim_{n \to \infty}\sum_{k}\int 1_{A_k}\cdot f_ndP$. Expressing each $f_n$ as single function we have: $f_n=\sum_{j=1}f_{n_j}\cdot 1_{B_{nj}}$. So $F(A_k)=\int\sum_j f_{n_j}\cdot 1_{B_{nj}}\cdot 1_{A_k}=\int\sum_j f_{n_j}\cdot 1_{B_{nj} \cap {A_k}}$
\end{enumerate}
\end{document}
