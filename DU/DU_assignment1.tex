...
\\\\
\textbf{Ex2.3)}\\
Show that if $\int gdP=F=\int fdP$ then $f=g$ a.e.
\\\\
We can use the result $\int fdP=0 \implies f=0$ a.e. We have by hypothesis that $\int (f-g)dP=0$, let $h=f-g$, using $h_+=$max$\{f,0\}$ and $h_-=$min$\{f,0\}$, we have $h=h_+-h_-$.
